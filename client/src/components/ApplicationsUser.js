...
const ApplicationsUser = () => {
  ...
  const getApps = async () => {
    try {
      const response = await axios.get("http://localhost:3001/GetApps", {
        // headers: {
        //   Authorization: `Bearer ${token}`
        // }
      });
      setApps(response.data);
      console.log("getapps complete");
  ...
  function EditApp(app) {
    ...
    setUser_group_Create({ value: app.App_Permit_Create, label: app.App_Permit_Create });
    setUser_group_Open({ value: app.App_Permit_Open, label: app.App_Permit_Open });
    setUser_group_ToDo({ value: app.App_Permit_Todolist, label: app.App_Permit_Todolist });
    setUser_group_Doing({ value: app.App_Permit_Doing, label: app.App_Permit_Doing });
    setUser_group_Done({ value: app.App_Permit_Done, label: app.App_Permit_Done });
    get_groupslist();
  }
  ...
  const get_groupslist = async () => {
    try {
      ...
      //console.log(response.data);
      console.log("getgrouplist complete");
    } catch (error) {
      if (error.response) {
  ...
  const updateApp = async e => {
    ...
    console.log(user_group_Create.label);
    console.log(Object.keys(user_group_Create).length);

    var user_group_create_str = "";
    if (Object.keys(user_group_Create).length && typeof user_group_Create !== "undefined") {
      user_group_create_str += user_group_Create.label;
    }
    console.log(user_group_create_str);

    var user_group_open_str = "";
    if (Object.keys(user_group_Open).length && typeof user_group_Open !== "undefined") {
      user_group_open_str += user_group_Open.label;
    }

    var user_group_ToDo_str = "";
    if (Object.keys(user_group_ToDo).length && typeof user_group_ToDo !== "undefined") {
      user_group_ToDo_str += user_group_ToDo.label;
    }

    var user_group_Doing_str = "";
    if (Object.keys(user_group_Doing).length && typeof user_group_Doing !== "undefined") {
      user_group_Doing_str += user_group_Doing.label;
    }

    var user_group_Done_str = "";
    if (Object.keys(user_group_Done).length && typeof user_group_Done !== "undefined") {
  ...
  const CreateApp = async e => {
    ...
    var user_group_Doing_str = "";
    var user_group_Done_str = "";
    console.log(user_group_Create);
    if (user_group_Create.length && typeof user_group_Create !== "undefined") {
      user_group_create_str += user_group_Create.label;
    }
    if (user_group_Open.length && typeof user_group_Open !== "undefined") {
      user_group_open_str += user_group_Open.label;
    }
    if (user_group_ToDo.length && typeof user_group_ToDo !== "undefined") {
      user_group_ToDo_str += user_group_ToDo.label;
    }
    if (user_group_Doing.length && typeof user_group_Doing !== "undefined") {
      user_group_Doing_str += user_group_Doing.label;
    }
    if (user_group_Done.length && typeof user_group_Done !== "undefined") {
      user_group_Done_str += user_group_Done.label;
    }
  ...
  async function App_Create() {
    ...
    console.log("APpCreate complete");
    //console.log(permit_app_create);
  }
            ...
            <tr key={app.id}>
              ...
              <td>
                ...
                <button
                  className="button is-small is-rounded is-link"
                  onClick={() => {
                    navigate(`/UserBoard/${user}/${app.App_Name}`);
                  }}
                >
                  View Board
                </button>
